...
const OrganizersSponsorsPage: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12">
      <section className="bg-surface p-8 rounded-xl shadow-xl border border-line/50 text-ink">
        <h1 className="text-2xl md:text-3xl font-bold text-center mb-6 text-brand-ink">Đơn vị tổ chức</h1>
        <div className="flex flex-col sm:flex-row items-center justify-center gap-8">
            ...
            <div className="text-center sm:text-left">
                ...
                <p className="text-ink-soft mt-1">Số 98 Dương Quảng Hàm, Quan Hoa, Cầu Giấy, Hà Nội.</p>
            </div>
        </div>
      </section>

      <section className="bg-surface p-8 rounded-xl shadow-xl border border-line/50 text-ink">
        <h1 className="text-2xl md:text-3xl font-bold text-center mb-8 text-brand-ink">Đơn vị tài trợ & Đối tác</h1>
        <div className="flex justify-center items-center gap-8 flex-wrap">
            {siteContent.sponsors.length === 0 ? (
              <p className="text-ink-muted italic">Đang cập nhật</p>
              ...
              <div key={sponsor.id} className="text-center p-4 border border-line rounded-lg hover:shadow-xl transition-shadow">
                ...
                <p className="mt-2 font-semibold text-ink-soft">{sponsor.name}</p>
              </div>
            ))}
        </div>
      </section>
