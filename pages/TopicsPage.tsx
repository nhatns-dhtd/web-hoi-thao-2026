...
const TopicsPage: React.FC = () => {
  return (
    <div className="max-w-4xl mx-auto bg-surface p-8 rounded-xl shadow-xl border border-line/50 text-ink">
      <h1 className="text-2xl md:text-3xl font-bold text-center mb-6 text-brand-ink">Chủ đề chính của Diễn đàn</h1>
      ...
      <div className="space-y-8">
        {conferenceTopics.map((topic, index) => (
          <section key={index}>
            <h2 className="text-xl font-bold text-brand-ink mb-3 flex items-start">
              <i className="fas fa-lightbulb text-amber-600 mt-1 mr-3"></i>
              <span>{index + 1}. {topic.title}</span>
            </h2>
            <ul className="space-y-2 ml-9">
              {topic.subTopics.map((subTopic, subIndex) => (
                <li key={subIndex} className="flex items-start p-3 bg-surface-sunken rounded-md">
                  <i className="fas fa-angle-right text-amber-700 mt-1.5 mr-3"></i>
                  <span className="text-ink-soft">{subTopic}</span>
                </li>
              ))}
            </ul>
          </section>
