...
const FeesPage: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto bg-slate-800/40 backdrop-blur-sm p-8 rounded-xl shadow-xl border border-slate-700/50 text-slate-100">
      <h1 className="text-2xl md:text-3xl font-bold text-center mb-6 text-teal-100">Lệ phí công bố, xuất bản Kỷ yếu</h1>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-900/60">
              <th className="p-4 border border-slate-700 font-semibold text-teal-200">Đối tượng</th>
              <th className="p-4 border border-slate-700 font-semibold text-teal-200 text-center">Lệ phí báo cáo toàn văn</th>
            </tr>
          </thead>
          <tbody>
            {feesData.map((item, index) => (
              <tr key={index} className="hover:bg-slate-900/50">
                <td className="p-4 border border-slate-700 font-medium">{item.category}</td>
                <td className="p-4 border border-slate-700 text-center">{item.fee}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="mt-8 space-y-4">
        ...
        <ul className="space-y-2 bg-slate-900/50 p-4 rounded-md border border-slate-700">
